...
Esta imagen representa un sistema formado por dos componentes principales:
- Asistente QA (orquestador): Interfaz de usuario de tipo interactivo que identifica al usuario, interpreta la petición del usuario redirigiendo  a un determinado agente especialista, procesa el resultado y captura feedback del usuario para implementar un proceso de mejora continua.
- Agente especialista en pruebas de ciclo del dato: implementa las etapas clásicas de percepción, planificación, ejecución, feedback, etc. en base a un determinado plan de pruebas formado por pasos  concretos como podría ser evaluación de nulos, métricas de evaluación de desempeño de un modelo, balance de clases en la división del dataset en train/validation/test, etc.

El asistente tendrá una interfaz web para interacción con usuarios testers de QA mientras que el agente especialista será accesible mediante una API para poder utilizarse desde el asistente y también desde un proceso de CI/CD de MLOps.

Ambos componentes hacen uso de un gestor de artefactos donde existirán espacios para que los usuarios suban datasets, modelos, o cualquier resultado del desarrollo que se utilizarán en CI/CD y por parte del agente especialista en QA de diclo del dato.

También se hace uso de un gestor de conocimiento donde están representadas en forma de grafo reglas que definen cómo identificar qué está pidiendo el usuario (percepción), qué pasos hay que implementar (planificación), la acción asociada a cada paso (ejecución), cómo interpretar el resultado (feedback), etc.
